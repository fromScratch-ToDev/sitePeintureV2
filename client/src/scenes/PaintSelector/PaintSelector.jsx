...
import {requestForPaint, getCategoryDescription, renameCategorie, deleteCategorie, addCategorie} from './api_bdd';
...
// Composant 
const PaintSelector = forwardRef(function PaintSelector({ buttonRadioSelected, setButtonRadioSelected, categoriesNames, setCategoriesNames, setPaintArray, isClosed, setIsClosed, setCategoryDescription}, ref) {
  // Style css
  ...
  useEffect(()=>{
    requestForPaint(buttonRadioSelected, categoriesNames, setPaintArray);
    getCategoryDescription(categoriesNames, buttonRadioSelected, setCategoryDescription)
  ...
  return (
    <section className={classes.container} ref={ref} >
      ...
      <Modal id="modalRename"
        header={
          <>
            ...
            <input id="inputChangeNameCategory" autoFocus type="text" value={inputValueChange} onChange={(e)=>setInputValueChange(e.target.value)} onKeyDown={(e)=> e.key === 'Enter' && renameCategorie(categoriesNames, buttonRadioSelected, setCategoriesNames, setInputValueChange, handleModalShowRename)} className={classes.input}></input>
          </>
        }
        buttonConfirm={
          <ButtonModal text="valider" f={()=>renameCategorie(categoriesNames, buttonRadioSelected, setCategoriesNames, setInputValueChange, handleModalShowRename)}></ButtonModal>
        }
        buttonCancel={
      ...
      <Modal id="modalDelete"
        ...
        buttonConfirm={
          <ButtonModal text="valider" f={()=>deleteCategorie(categoriesNames, buttonRadioSelected, setCategoriesNames, setButtonRadioSelected, handleModalShowDelete)}></ButtonModal>
        }
        buttonCancel={
      ...
      <Modal id="modalAdd"
        header={
          <>
            ...
            <input id="inputNameNewCategory" autoFocus type="text" value={inputValueAdd} onChange={(e)=>setInputValueAdd(e.target.value)}  onKeyDown={(e)=> e.key === 'Enter' && addCategorie(categoriesNames, setCategoriesNames)} className={classes.input}></input>
          </>
          } 
          buttonConfirm={
            <ButtonModal text="valider" f={()=>addCategorie(categoriesNames, setCategoriesNames)}></ButtonModal>
          }
          buttonCancel={
